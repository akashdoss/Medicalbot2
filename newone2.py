...
def recognize_speech_and_search():
    ...
    try:
        ...
        if text:
            ...
            translated_response = translate_text(response)  # Translate the response to Tamil
            st.write(f"👨‍⚕️ MedoBot: {response}")  # Add assistant-like response
            st.write(f"👨‍⚕️ MedoBot (Tamil): {translated_response}")  # Display translated response
...
def translate_text(text):
    translated = translator.translate(text, dest='ta')  # 'ta' is the language code for Tamil
...
if st.button("💬 Get Medicine Information"):
    if medicine_name:
        ...
        try:
            query = f"Provide a brief and simple summary of {medicine_name} that anyone can easily understand."
            response = get_medicine_info_from_gemini(medicine_name)

            translated_response = translate_text(response)  # Translate to Tamil before displaying
            
            progress_bar.progress(100)  # Complete loading
            
            # Display English response and its translation in Tamil immediately below it.
            st.write(response)
            st.write(f"👨‍⚕️ MedoBot (Tamil): {translated_response}")
...
if st.sidebar.button("🏥 Find Nearby Hospitals"):
    if location:
        try:
            ...
            translated_hospital_info = translate_text(hospital_info)  # Translate hospital info to Tamil 
            
            st.sidebar.write("Nearby Hospitals:")
            st.sidebar.write(hospital_info)  # Display hospital info in English 
